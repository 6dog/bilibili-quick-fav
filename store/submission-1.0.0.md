# Chrome Web Store 1.0 正式版提交资料

## 冻结包

- 技术版本号：`1.0.0`；对外称“1.0 正式版”。
- 上传文件：`dist/release/bilibili-quick-fav-1.0.0.zip`
- SHA-256：`1498bf5924aa9ec288a6d0d42ef9270bf8ea165621ad8a9730943fc80611193c`
- ZIP 内 10 个文件：`BUILD_VERSION`、`content.js`、`manifest.json`、4 个 PNG 图标、`popup/index.html`、`popup/popup.css`、`popup/popup.js`。
- 正式版修复窗口重新获得焦点、其他标签页更改收藏夹设置或播放器尺寸异步变化后详情按钮隐藏的问题。
- 最终 ZIP 已完成只读浏览器验收，以及真实收藏加入与撤销验收：预检全部目录后，只更改一个目标收藏夹，随后撤销并确认目标及其他目录都恢复原状态。
...
- 公开，全部支持地区。
- 提交审核时保留“审核通过后自动发布”，审核通过后向全部支持地区公开。
- 2026-09-18 已在 Chrome 应用商店开发者后台确认发布者账号可用，公开联系邮箱已通过验证。
- 条目 ID：`gdbopjlnapleckkjeojjmcmfnnakphhf`。
- 2026-09-20 已上传 1.0.0 冻结包，并保存商品详情、图片素材、隐私披露、公开且全部地区的分发设置和审核测试说明。
- 2026-09-20 已提交审核；确认弹窗中的“通过审核后自动发布”保持选中。
- 当前状态：`待审核`。Google 尚未确认发布，因此当前不视为已上线。
